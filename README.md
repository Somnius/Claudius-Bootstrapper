...
| Backend | Typical mistake | What Claudius does (0.9.10+) |
|--------|------------------|------------------------------|
...
- **Windows: `claudius.ps1` parse errors** (“Expressions are only allowed as the first element of a pipeline”, “Missing closing `}`”, `Write-Host` / `)` errors) – Fixed in **v0.9.10**: use **`git pull`** so `claudius.ps1` uses `String.Split('|')` instead of `-split '\|'` (PS 5.1 misparsed the backslash–pipe in single quotes). See changelog **0.9.10**.
