...
| **Config & shell** | Writes `~/.claude/settings.json` (base URL, auth, defaultModel) and appends ANTHROPIC_* to your shell config. For **custom/OpenRouter** only `ANTHROPIC_API_KEY` is set; for **LM Studio/Ollama** only `ANTHROPIC_AUTH_TOKEN`, so Claude Code never sees both (avoids “auth conflict”). |
