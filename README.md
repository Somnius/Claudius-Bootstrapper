...
| Backend | Typical mistake | What Claudius does (0.9.11+) |
|--------|------------------|------------------------------|
...
- **Windows: `claudius.ps1` parse errors** (“Expressions are only allowed as the first element of a pipeline”, “Missing closing `}`”, `Write-Host` / `)` errors) – Fixed in **v0.9.11**: **`git pull`** — runtime code uses `String.Split` (since **0.9.10**), and **0.9.11** removes `'\|'`-style text from comments (those sequences still confuse the lexer on **PS 5.1** and **PS 7**). See changelog **0.9.11**.
